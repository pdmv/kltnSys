<%-- 
    ...
    Created on : May 29, 2024, 4:19:50 PM
...
<h2 class="text-center m-2">Cập nhật <strong>GIẢNG VIÊN</strong></h2>

<c:if test="${errorMessage != null}">
    <div class="alert alert-danger" role="alert">${errorMessage}</div>
</c:if>

<c:url value="/admins/lecturers/${id}" var="act"/>
<form:form method="post" action="${act}" modelAttribute="lecturer" enctype="multipart/form-data" onsubmit="return validatePassword()">
    <form:hidden path="id"/>
    ...
    <c:if test="${!empty lecturer.accountId.avatar}">
    <div class="d-flex justify-content-center mb-3">
        <img src="${lecturer.accountId.avatar}" class="rounded-circle" alt="Ảnh đại diện" height="150" width="150">
    </div>
    </c:if>
    ...
    <div class="form-floating mb-3">
        <form:input path="degree" type="text" class="form-control" id="degree" placeholder="Trình độ" required="required"/>
        <label for="degree">Trình độ</label>
        <form:errors path="degree" cssClass="text-danger"/>
    </div>
    
    <div class="form-floating mb-3" id="facultyInput">
        <form:select path="facultyId" class="form-control">
            <c:forEach items="${faculties}" var="faculty">
                <option value="${faculty.id}" <c:if test="${faculty.id eq lecturer.facultyId.id}">selected</c:if>>${faculty.name}</option>
            </c:forEach>
        </form:select>
    ...
    <a type="button" class="btn btn-outline-dark" href="<c:url value="/admins/lecturers"/>">Quay lại</a>
</form:form>
